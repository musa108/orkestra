...
export function DecisionReviewModal({
  ...
  const summary = changes.summary || decision.comments || 'Authorization required for automated workflow progression.';
  ...
  const handleAction = async (type: 'approve' | 'reject') => {
    ...
    setActionType(type);
    try {
      if (type === 'approve') await onApprove(decision.id);
      else await onReject(decision.id);
      onClose();
    } finally {
      setSubmitting(false);
      setActionType(null);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-xs animate-in fade-in duration-150">
      <div className="bg-card text-card-foreground w-full max-w-2xl rounded-lg border border-border shadow-2xl overflow-hidden divide-y divide-border">
        {/* Modal Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-sm font-bold text-foreground">AI Decision Review Gate</h2>
                <span className={clsx(
        ...
        <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto text-xs">
          ...
          <div className="space-y-1">
            ...
            <div className="p-3 rounded bg-muted/40 border border-border text-foreground font-medium leading-relaxed">
              {summary}
            </div>
          </div>

          {/* AI Recommendation Box */}
          <div className="p-3.5 rounded bg-emerald-50/70 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800/50 space-y-1">
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-2">
              {evidence.map((ev, idx) => (
                <div key={idx} className="p-2.5 rounded bg-muted/30 border border-border flex items-start gap-2.5">
                  ...
                  <div className="min-w-0">
                    <p className="font-semibold text-foreground">{ev.factor?.replace(/_/g, ' ')}</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
            <div className="p-3 rounded bg-muted/20 border border-border space-y-1.5">
              <span className="text-[10px] font-mono text-muted-foreground uppercase tracking-wider block">
                Contributing Factors:
              </span>
              <ul className="list-disc list-inside space-y-0.5 text-foreground">
                {factors.map((fac, i) => (
                  <li key={i} className="text-[11px]">{fac}</li>
                ))}
              </ul>
            </div>
          ...
          <div className="p-3 rounded bg-muted/40 border border-border flex items-center justify-between text-[11px] text-muted-foreground">
            <span>Data Sources: <strong>ClickHouse Historical Intelligence + FastMCP + Gemini 2.0</strong></span>
            <span className="font-mono">Gate: Separation of Duties Active</span>
          </div>
        </div>

        {/* Modal Footer Actions */}
        <div className="p-4 bg-muted/20 flex items-center justify-end gap-3">
          <button
